...
def get_args():
    parser = argparse.ArgumentParser(description="Pass text into command line or specify a json/yaml file")
    parser.add_argument('-t', '--text', dest='input_text', help="Pass input directly")
    parser.add_argument('-f', '--file', dest='input_file', help="Specify path of input json/yaml file")
...
def print_employers(data, indent=0):
    ...
    if data['employer']['reports']:
        ...
        for emp in sorted(data['employer']['reports'], key=lambda x: x['employer']['name']):
